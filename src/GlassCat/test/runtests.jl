...
@test_throws ErrorException Optics.GlassCat.parse_glass_file!(Dict(), "NOFILE")

cat = Dict()

@testset "Parsing Tests" begin
    Optics.GlassCat.parse_glass_file!(cat, "./TEST_CAT.AGF")
    ...
    @test Optics.GlassCat.load_glass_db(".") == cat
end

@testset "Module Gen Tests" begin
    Optics.GlassCat.generate_cat_jl(cat, "./TEST_CAT.jl")
    include("./TEST_CAT.jl")
...
@testset "Glass Tests" begin
    ...
    @test index(Optics.GlassCat.Air, 500 * u"nm") == 1.0
    @test index(Optics.GlassCat.Air, 600 * u"nm") == 1.0
    @test isapprox(index(Optics.GlassCat.Vacuum, 500 * u"nm"), 0.9997258841958212, atol = 1e-14)

    # test against true values
    g = Optics.GlassCat.SCHOTT.N_BK7
    ...
    # test transmission
    @test absorption(Optics.GlassCat.Air, 500 * u"nm") == 0.0
    @test absorption(Optics.GlassCat.Air, 600 * u"nm") == 0.0
    ...
    @test (@allocated Optics.GlassCat.SCHOTT.N_BK7) == 0

    @test glassforid(glassid(Optics.GlassCat.SCHOTT.N_BK7)) == Optics.GlassCat.SCHOTT.N_BK7

    @test isair(Optics.GlassCat.Air) == true
    @test isair(Optics.GlassCat.SCHOTT.N_BK7) == false

    # test MIL and model glasses
    fit_acc = 0.001
    bk7 = Optics.GlassCat.glassfromMIL(517642)
    ...
    t2 = Optics.GlassCat.glassfromMIL(1.135635)
    @test isapprox(index(t2, 0.5875618), 2.135, atol = fit_acc)

    fit_acc = 0.0001
    bk7 = Optics.GlassCat.modelglass(1.5168, 64.167336, -0.0009)
    @test glassforid(glassid(bk7)) == bk7
    ...
    # test other glass
    @test index(Optics.GlassCat.CARGILLE.OG0608, 0.578) == 1.4596475735607324

    # make sure that the other functions work
    plot_indices(Optics.GlassCat.SCHOTT.N_BK7; polyfit = true, fiterror = true)
end
