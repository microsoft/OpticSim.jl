...
function add_agf(
    ...
    if isnothing(name)
        ...
        if isnothing(match(r"^([a-zA-Z]+)$", name))
...
"""
    download_source(agffile::AbstractString, url::AbstractString, POST_data::Maybe{AbstractString} = nothing)

Download and unzip an AGF glass catalog from a publicly available source. Supports POST requests.
"""
function download_source(agffile::AbstractString, url::AbstractString, POST_data::Maybe{AbstractString} = nothing)
