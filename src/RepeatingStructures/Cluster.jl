...
Cluster()"""
struct Cluster{N1, B1{N,T<:Real}<:Basis{N,T},B2{N,T<:Real}<:Basis{N,T}}
    clusterbasis::B1{N,T}
    elementbasis::B2{N,T}
...
function Base.getindex(A::Cluster{N1,B1{N,T},B2{N,T}}, indices::Vararg{Int, N}) where{N} 
...
function Base.size(a::Cluster{N,B1{N,T},B2{N,T}}) where{N,T}
