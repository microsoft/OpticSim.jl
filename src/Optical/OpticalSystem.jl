...
```julia
CSGOpticalSystem(assembly::LensAssembly, detector::Surface, detectorpixelsx = 1000, detectorpixelsy = 1000, ::Type{D} = Float32; temperature = Optics.GlassCat.TEMP_REF, pressure = Optics.GlassCat.PRESSURE_REF)
```
"""
struct CSGOpticalSystem{T,D<:Number,S<:Surface{T},L<:LensAssembly{T}} <: OpticalSystem{T}
    ...
    temperature::T
    pressure::T
    function CSGOpticalSystem(assembly::L, detector::S, detectorpixelsx::Int = 1000, detectorpixelsy::Int = 1000, ::Type{D} = Float32; temperature::Union{T,Unitful.Temperature} = T(Optics.GlassCat.TEMP_REF), pressure::T = T(Optics.GlassCat.PRESSURE_REF)) where {T<:Real,S<:Surface{T},L<:LensAssembly{T},D<:Number}
...
```julia
AxisymmetricOpticalSystem{T}(prescription::DataFrame, detectorpixelsx = 1000, detectorpixelsy:: = 1000, ::Type{D} = Float32; temperature = Optics.GlassCat.TEMP_REF, pressure = Optics.GlassCat.PRESSURE_REF)
```
"""
struct AxisymmetricOpticalSystem{T,C<:CSGOpticalSystem{T}} <: OpticalSystem{T}
    ...
    function AxisymmetricOpticalSystem{T}(prescription::DataFrame, detectorpixelsx::Int = 1000, detectorpixelsy::Int = 1000, ::Type{D} = Float32; temperature::Union{T,Unitful.Temperature} = T(Optics.GlassCat.TEMP_REF), pressure::T = T(Optics.GlassCat.PRESSURE_REF)) where {T<:Real,D<:Number}
        pr = prescription
        ...
        let sumstart = 1
            ...
            for i in 1:(rows - 1)
                if pr[i, :Surface] == :Stop
                    ...
                    if material != Optics.GlassCat.Air && !isequal(material, missing)
            ...
            system = CSGOpticalSystem(Optics.LensAssembly(elements...), det, detectorpixelsx, detectorpixelsy, D, temperature = temperature, pressure = pressure)
...
function traceMT(system::CSGOpticalSystem{T,S}, raygenerator::OpticalRayGenerator{T}; printprog::Bool = true, test::Bool = false, outpath::Union{Nothing,String} = nothing) where {T<:Real,S<:Number}
    ...
    det = Optics.detectorimage(copies[1][1])
    # sum all the copies
    @inbounds for i in 2:N
        Optics.sum!(det, Optics.detectorimage(copies[i][1]))
    end
