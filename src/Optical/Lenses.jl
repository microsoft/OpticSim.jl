...
function AsphericLens(
    ...
    frontradius::T,
    frontconic::T,
    frontaspherics::Union{Nothing,Vector{Tuple{Int,T}}},
    backradius::T,
    backconic::T,
    backaspherics::Union{Nothing,Vector{Tuple{Int,T}}},
    thickness::T,
    semidiameter::T;
    lastmaterial::AbstractGlass = Air,
    nextmaterial::AbstractGlass = Air,
    ...
    elseif backconic == zero(T) && backaspherics === nothing
        ...
        if backradius < zero(T)
            ...
            if backradius == semidiameter
                ...
                plane = leaf(Plane(SVector{3,T}(0, 0, 1), SVector{3,T}(0, 0, frontvertex - thickness + backradius)))
        ...
        if abs(backradius) - backdecenter_l <= semidiameter
            ...
            lens_rear = backradius < zero(T) ? csgunion(lens_rear, plane) : csgintersection(lens_rear, plane)
        end
    else
        #conic or aspheric
        surf = AcceleratedParametricSurface(
...
function FresnelLens(
    ...
    if conic == zero(T)
        ...
        while true
            ...
            if radius > zero(T)
                ...
                newsurf = csgdifference(
                    leaf(sphere, translation(T, zero(T), zero(T), frontvertex - radius + offset)),
                    cylinder
                )
    ...
    fresnel = (
        reverse ?
        csgintersection(backplane, fresnel) :
        leaf(fresnel, rotationd(T, zero(T), convert(T, 180), zero(T)))
    )
    return fresnel
end
